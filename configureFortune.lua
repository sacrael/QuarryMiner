...
local DB_FORTUNE_SLOT_END = config.DB.FORTUNE_SLOTS.END

local function findFirstAvailableSlot ()
    for i = DB_FORTUNE_SLOT_START, DB_FORTUNE_SLOT_END do
...
function ConfigureFortune.beginConfiguration (block_count) 
    ...
    if slot_start < 0 then
        error("No available slots in database between ".. DB_FORTUNE_SLOT_START .. " and " .. DB_FORTUNE_SLOT_END)
        return
    end
    ...
    for i=1, robot.inventorySize() do 
        ...
        if slot_start <= DB_FORTUNE_SLOT_END 
        ...
        elseif slot_start > DB_FORTUNE_SLOT_END then
